...
// Outputs: none
void ST7735_PlotPoint(int32_t x,int32_t y){int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  ...
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel(k,   j,   ST7735_BLUE);
  ST7735_DrawPixel(k+1, j,   ST7735_BLUE);
  ST7735_DrawPixel(k,   j+1, ST7735_BLUE);
  ST7735_DrawPixel(k+1, j+1, ST7735_BLUE);
}
// *************** ST7735_PlotLine ********************
